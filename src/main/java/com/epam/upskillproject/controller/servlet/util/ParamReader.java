...
public class ParamReader {
    ...
    public Optional<TargetType> readTargetType(HttpServletRequest req, String paramName) {
        Optional<String> paramValue = readString(req, paramName);
        TargetType target;
        if (paramValue.isPresent()) {
            try {
                target = TargetType.valueOf(paramValue.get().toUpperCase());
            } catch (IllegalArgumentException e) {
                return Optional.empty();
            }
        } else {
    ...
    public Optional<TargetType> readTargetTypeParamOrAttr(HttpServletRequest req, String paramName) {
        Optional<TargetType> paramTarget = readTargetType(req, paramName);
        if (paramTarget.isPresent()) {
            return paramTarget;
        } else if (req.getAttribute(paramName) instanceof String) {
            TargetType attrTarget;
            try {
                attrTarget = TargetType.valueOf(
                        ((String) req.getAttribute(paramName)).toUpperCase()
                );
                return Optional.of(attrTarget);
            } catch (IllegalArgumentException e) {
                return Optional.empty();
            }
        } else {
