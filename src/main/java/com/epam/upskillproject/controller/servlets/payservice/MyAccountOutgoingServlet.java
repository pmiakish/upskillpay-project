...
@WebServlet("/payservice/my_account_outgoing/*")
@ServletSecurity(@HttpConstraint(rolesAllowed = {"CUSTOMER"}))
public class MyAccountOutgoingServlet extends HttpServlet {

    private static final Logger logger = LogManager.getLogger(MyAccountOutgoingServlet.class.getName());
    ...
    private BigInteger getIdFromRequestUri(HttpServletRequest req) throws IllegalArgumentException {
        ...
        String urlId = path.replace("/payservice/my_account_outgoing/", "");
        BigInteger id;
        try {
