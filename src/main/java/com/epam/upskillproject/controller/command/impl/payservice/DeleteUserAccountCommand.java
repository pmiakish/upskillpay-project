...
@Singleton
public class DeleteUserAccountCommand extends AbstractCommand {

    private static final Logger logger = LogManager.getLogger(DeleteUserAccountCommand.class.getName());
    ...
    @Inject
    public DeleteUserAccountCommand(LocaleDispatcher localeDispatcher, ParamReader paramReader,
            ...
            try {
                ...
                if (user != null) {
                    ...
                    if (accountId.isPresent()) {
                        boolean deleted = customerService.deleteUserAccount(principal, accountId.get());
                        ...
                        logger.log(Level.WARN, String.format("Account removal failed (principal: %s, accountId: %s)",
                                principal.getName(), accountId.orElse(null)));
                        setOperationError(req, OperationType.DELETE, "Cannot delete an account (id not " +
                                "passed). ");
                ...
                logger.log(Level.WARN, String.format("Cannot delete user's account (principal: %s)", principal.getName()),
                        e);
                setOperationError(req, OperationType.DELETE, "Cannot delete user's account (internal server " +
                        "error). ");
                commandResult = new CommandResult(false, HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            } catch (TransactionException e) {
                logger.log(Level.WARN, String.format("Cannot delete account (uri: %s)", req.getRequestURI()));
