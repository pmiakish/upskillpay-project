...
@Singleton
public class CardBlockCommand extends AbstractCommand {

    private static final Logger logger = LogManager.getLogger(CardBlockCommand.class.getName());
    ...
    @Inject
    public CardBlockCommand(LocaleDispatcher localeDispatcher, ParamReader paramReader, AdminService adminService) {
    ...
    public CommandResult execute(HttpServletRequest req, HttpServletResponse resp) {
        ...
        if (id.isPresent()) {
            try {
                boolean updated = adminService.updateCardStatus(id.get(), StatusType.BLOCKED);
                ...
                logger.log(Level.WARN, String.format("Cannot block card (id: %s)", id.orElse(null)), e);
                setOperationError(req, OperationType.UPDATE, "Cannot block card (internal server error). ");
                commandResult = new CommandResult(false, HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
            }
        } else {
            logger.log(Level.WARN, "Cannot block card (id is not passed)");
